...
#When you finally find out what John's login is, use `proton(action = "login", login="XYZ")` command, where XYZ is Insecure's login.John
john_login <- employees[employees$surname == "Insecure", "login"]

proton(action = "login", login = john_login[1])
...
#  `Password or login is incorrect!`
  ...
  for (i in top1000passwords) {
    proton(action = "login", login=john_login, password=i)
  }
...
Pietruszko_login <- employees[employees$surname == "Pietraszko", "login"]
pietruszka_entry <- logs[logs$login == Pietruszko_login,]
host_counts <- table(pietruszka_entry$host)


# View the result
xd <- host_counts[host_counts==max(host_counts)]
...
for (i in bash_history) {
  proton(action = "login", login=Pietruszko_login, password=i)
}
